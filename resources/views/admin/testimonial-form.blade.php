<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Testimonial — Stoka Admin</title><link rel="preconnect" href="https://fonts.googleapis.com"><link href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:wght@400;500;600&family=Plus+Jakarta+Sans:wght@400;500;600&family=DM+Mono:wght@400;500&display=swap" rel="stylesheet"><style>*,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
...
<div class="main">
  ...
  <div class="page-content">
    ...
    <form method="POST" action="@if($testimonial)/admin/testimonials/{{ $testimonial->id }}@else/admin/testimonials@endif">
      @csrf
      <div class="form-card">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
          <div class="field"><label>Name</label><input type="text" name="name" value="{{ old('name', $testimonial->name ?? '') }}" required></div>
          <div class="field"><label>Location <span style="font-weight:400;text-transform:none;letter-spacing:0;">(e.g. Zawadi Boutique · Nairobi)</span></label><input type="text" name="location" value="{{ old('location', $testimonial->location ?? '') }}" required></div>
        </div>
        <div class="field">
          <label>Pull quote <span style="font-weight:400;text-transform:none;letter-spacing:0;">(the big headline quote shown on the homepage)</span></label>
          <textarea name="pull_quote" rows="3" required>{{ old('pull_quote', $testimonial->pull_quote ?? '') }}</textarea>
        </div>
        <div class="field">
          <label>Full testimonial body</label>
          <textarea name="body" rows="6" required>{{ old('body', $testimonial->body ?? '') }}</textarea>
        </div>
        <div class="field">
