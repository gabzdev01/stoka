<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Testimonials — Stoka Admin</title><link rel="preconnect" href="https://fonts.googleapis.com"><link href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:wght@400;500;600&family=Plus+Jakarta+Sans:wght@400;500;600&family=DM+Mono:wght@400;500&display=swap" rel="stylesheet"><style>*,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
...
<div class="main">
  <div class="page-header">
    <div><h1 class="page-title">Testimonials</h1><p class="page-sub">The first published testimonial appears on the homepage.</p></div>
    <a href="/admin/testimonials/create" class="btn-primary">+ New Testimonial</a>
  </div>
    ...
    <div class="table-card">
      ...
      <div class="row-item">
        <div style="flex:1;min-width:0;">
          <div class="row-title">{{ $t->name }} &middot; {{ $t->location }}</div>
          <div class="row-sub" style="margin-top:4px;font-style:italic;">&ldquo;{{ Str::limit($t->pull_quote, 80) }}&rdquo;</div>
          <div style="margin-top:4px;"><span class="badge {{ $t->published ? 'b-on' : 'b-off' }}">{{ $t->published ? 'Published' : 'Draft' }}</span></div>
        </div>
        <div class="row-actions">
          <a href="/admin/testimonials/{{ $t->id }}/edit" class="btn-ghost" style="font-size:11px;">Edit</a>
          <form method="POST" action="/admin/testimonials/{{ $t->id }}/delete" onsubmit="return confirm('Delete?')">
            @csrf<button type="submit" class="btn-danger" style="font-size:11px;">Delete</button>
          </form>
        </div>
      </div>
      @empty<div style="padding:40px;text-align:center;color:var(--muted);">No testimonials yet.</div>@endforelse
    </div>
  </div>
</div></div></body></html>