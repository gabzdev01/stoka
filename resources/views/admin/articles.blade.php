<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Articles — Stoka Admin</title><link rel="preconnect" href="https://fonts.googleapis.com"><link href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:wght@400;500;600&family=Plus+Jakarta+Sans:wght@400;500;600&family=DM+Mono:wght@400;500&display=swap" rel="stylesheet"><style>*,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
...
<div class="main">
  <div class="page-header">
    <div><h1 class="page-title">Articles</h1><p class="page-sub">Insights published on the marketing site. Drag sort_order to reorder.</p></div>
    <a href="/admin/articles/create" class="btn-primary">+ New Article</a>
  </div>
    ...
    <div class="table-card">
      ...
      <div class="row-item">
        <div style="flex:1;min-width:0;">
          ...
          <div class="row-sub" style="display:flex;gap:8px;align-items:center;margin-top:4px;">
            <span style="font-family:DM Mono,monospace;font-size:10px;color:var(--muted);">/insights/{{ $a->slug }}</span>
            <span class="badge {{ $a->published ? 'b-on' : 'b-off' }}">{{ $a->published ? 'Published' : 'Draft' }}</span>
            <span style="font-size:10px;color:var(--muted);">Order: {{ $a->sort_order }}</span>
          </div>
        </div>
        <div class="row-actions">
          <a href="https://tempforest.com/insights/{{ $a->slug }}" target="_blank" class="btn-ghost" style="font-size:11px;">Preview →</a>
          <a href="/admin/articles/{{ $a->id }}/edit" class="btn-ghost" style="font-size:11px;">Edit</a>
          <form method="POST" action="/admin/articles/{{ $a->id }}/delete" onsubmit="return confirm('Delete this article?')">
            @csrf<button type="submit" class="btn-danger" style="font-size:11px;">Delete</button>
          </form>
        </div>
      </div>
      @empty<div style="padding:40px;text-align:center;color:var(--muted);">No articles yet.</div>@endforelse
    </div>
  </div>
</div></div></body></html>