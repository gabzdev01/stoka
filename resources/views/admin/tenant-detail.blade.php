<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>{{ $tenant->name }} — Stoka Admin</title><link rel="preconnect" href="https://fonts.googleapis.com"><link href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:wght@400;500;600&family=Plus+Jakarta+Sans:wght@400;500;600&family=DM+Mono:wght@400;500&display=swap" rel="stylesheet"><style>*,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
...
<div class="main">
  ...
  <div class="page-content">
    ...
    <div style="display:grid;grid-template-columns:260px 1fr;gap:20px;align-items:start;">
      <div>
        <div style="background:white;border:1px solid var(--border);border-radius:var(--radius-default);overflow:hidden;box-shadow:0 1px 3px rgba(28,24,20,0.04);margin-bottom:16px;">
          <div style="padding:12px 18px;background:var(--surface);border-bottom:1px solid var(--border);font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;color:var(--muted);">Shop info</div>
          <div style="padding:4px 18px 8px;">
            @foreach([['Owner',$tenant->owner_name],['Phone',$tenant->owner_phone],['Plan',strtoupper($tenant->plan ?? '')],['Status',ucfirst($tenant->status ?? '')],['Currency',$tenant->currency ?? ''],['Shop page',$tenant->shop_enabled?'Live':'Off'],['Owner last seen',$detail['owner_last_seen'] ? \Carbon\Carbon::parse($detail['owner_last_seen'])->diffForHumans() : '—'],['Created',\Carbon\Carbon::parse($tenant->created_at)->format('d M Y')]] as [$lbl,$val])
            <div style="display:flex;justify-content:space-between;align-items:baseline;padding:8px 0;border-bottom:1px solid var(--border);">
              ...
              <span style="font-size:12px;font-weight:500;color:var(--espresso);max-width:140px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;text-align:right;">{{ $val }}</span>
            </div>
            @endforeach
          </div>
        </div>
        <div style="background:white;border:1px solid var(--border);border-radius:var(--radius-default);overflow:hidden;box-shadow:0 1px 3px rgba(28,24,20,0.04);margin-bottom:16px;">
          <div style="padding:12px 18px;background:var(--surface);border-bottom:1px solid var(--border);font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;color:var(--muted);">At a glance</div>
          <div style="padding:14px;display:grid;grid-template-columns:1fr 1fr;gap:10px;">
            @foreach([['Products',$detail['products']['active']],['Inactive',$detail['products']['inactive']],['Open credit',number_format($detail['open_credit'])],['Total shifts',$detail['total_shifts']]] as [$l,$v])
            <div style="background:var(--surface);border-radius:var(--radius-md);padding:12px 14px;"><div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.07em;color:var(--muted);margin-bottom:4px;">{{ $l }}</div><div style="font-family:DM Mono,monospace;font-size:20px;font-weight:500;color:var(--espresso);">{{ $v }}</div></div>
            @endforeach
          </div>
        </div>
        @if($tenant->id !== 'demo')
        <div style="background:white;border:1px solid var(--border);border-radius:var(--radius-default);overflow:hidden;box-shadow:0 1px 3px rgba(28,24,20,0.04);">
          <div style="padding:12px 18px;background:var(--surface);border-bottom:1px solid var(--border);font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;color:var(--muted);">Quick actions</div>
          <div style="padding:14px;display:flex;flex-direction:column;gap:8px;">
            <a href="https://wa.me/{{ preg_replace('/[^0-9]/','', $tenant->owner_phone) }}?text={{ urlencode('Hi '.$tenant->owner_name.', checking in from Stoka') }}" target="_blank" style="display:flex;align-items:center;justify-content:center;gap:7px;padding:9px 14px;background:#25D366;color:white;border-radius:var(--radius-md);font-size:12px;font-weight:600;text-decoration:none;"><svg viewBox="0 0 24 24" fill="currentColor" width="13" height="13"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/></svg> WhatsApp owner</a>
            <form method="POST" action="/admin/tenants/{{ $tenant->id }}/reset-password" onsubmit="return confirm('Reset password?')">@csrf<button type="submit" style="width:100%;padding:9px;background:white;border:1px solid var(--border);border-radius:var(--radius-md);font-family:inherit;font-size:12px;font-weight:500;color:var(--espresso);cursor:pointer;">Reset owner password</button></form>
            <form method="POST" action="/admin/tenants/{{ $tenant->id }}/shop-toggle">@csrf<button type="submit" style="width:100%;padding:9px;background:white;border:1px solid var(--border);border-radius:var(--radius-md);font-family:inherit;font-size:12px;font-weight:500;color:var(--espresso);cursor:pointer;">{{ $tenant->shop_enabled?'Disable shop':'Enable shop' }}</button></form>
            <form method="POST" action="{{ route('admin.tenants.toggle',$tenant->id) }}" onsubmit="return confirm('Sure?')">@csrf<button type="submit" style="width:100%;padding:9px;background:{{ $tenant->status==='active'?'#F8E8E4':'white' }};border:1px solid {{ $tenant->status==='active'?'#E8C8C0':'var(--border)' }};border-radius:var(--radius-md);font-family:inherit;font-size:12px;font-weight:500;color:{{ $tenant->status==='active'?'var(--clay)':'var(--espresso)' }};cursor:pointer;">{{ $tenant->status==='active'?'Suspend tenant':'Activate tenant' }}</button></form>
          </div>
        </div>
        @endif
      </div>
      <div>
        <div style="background:white;border:1px solid var(--border);border-radius:var(--radius-default);overflow:hidden;box-shadow:0 1px 3px rgba(28,24,20,0.04);margin-bottom:16px;">
          <div style="padding:12px 18px;background:var(--surface);border-bottom:1px solid var(--border);display:flex;justify-content:space-between;align-items:center;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;color:var(--muted);">
          ...
          <div style="padding:20px 24px;">
            <div style="display:flex;align-items:flex-end;gap:10px;height:100px;">
              @foreach($detail['monthly_sales'] as $ms)@php $pct=$maxSale>0?round($ms->total/$maxSale*100):0;@endphp
              <div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:4px;height:100%;">
                <div style="font-family:DM Mono,monospace;font-size:9px;color:var(--muted);">{{ $ms->total>0?number_format($ms->total/1000,0).'k':'' }}</div>
                <div style="flex:1;width:100%;display:flex;align-items:flex-end;"><div style="width:100%;background:var(--terracotta);border-radius:4px 4px 0 0;height:{{ max($pct,2) }}%;opacity:0.75;" title="{{ number_format($ms->total) }}"></div></div>
                <div style="font-size:9px;color:var(--muted);white-space:nowrap;">{{ substr($ms->month,0,3) }}</div>
              </div>
              @endforeach
            </div>
          </div>
